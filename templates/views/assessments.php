...
<body style="background-color:#060738;">
  <div style="display:flex;align-items:center;justify-content:center;min-height:80vh; flex-wrap: wrap; gap: 2%; margin-top: 4%">
    <?php if (empty($movieList)) : ?>
      <h1 style="color: white;"><?= "Você ainda não avaliou nenhum filme!" ?></h1>
    <?php endif; ?>
    <?php foreach ($movieList as $film) : ?>
      <div class="card" style="width: 18rem; margin-bottom: 2%; background-color: <?= $film->getUserGrade() == 1 ? 'green' : 'red' ?>;">
        ...
        <div class="card-body">
          ...
          <form action="http://moviematch.com/assessments" method="POST">
            ...
            <button class="btn btn-danger" style="width: 95%;">Remover avaliação</button>
          </form>
        </div>
      </div>
    <?php endforeach; ?>
